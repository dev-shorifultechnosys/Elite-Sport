(() => {
  ...
  document.addEventListener('keydown', e => { if (e.key === 'Escape') { closeMenu(); setTimeout(() => document.querySelector('.chat-panel')?.classList.contains('open') && document.querySelector('.chat-close')?.click(), 0); } });
  ...
  chatToggle?.addEventListener('click', () => setChat(chatToggle.getAttribute('aria-expanded') !== 'true'));
  chatClose?.addEventListener('click', () => setChat(false));
  chatStart?.addEventListener('click', () => showToast('Demo: Der Kundenservice-Chat ist für die spätere Anbindung vorbereitet.'));
  document.addEventListener('click', e => {
    if (chat && chatPanel?.classList.contains('open') && !chat.contains(e.target)) setChat(false);
  });
